...
`--serve` runs a small range-capable server (`engine/preview_server.py`) rather than Python's stock one. That matters the moment you have audio: the stock server answers a Range request with the whole file, browsers read that as "seeking not supported", and your episode player's timeline stops responding while being perfectly correct.
