...
def htaccess_content(redirects=()):
    ...
    # Configured redirects come first: they are the migration ones, they name
    # exact old paths, and a later engine rule must never get to a URL one of
    # these was written to rescue.
    configured = ""
    if redirects:
    ...
    return (
        ...
        "\n"
        "    # WordPress-shaped post URLs: /<slug>/ -> /posts/<slug>.html. The\n"
        "    # shape a migrated site arrives with, and the one search engines\n"
        "    # already have indexed. Existence-checked like the rule above, so a\n"
        "    # single-segment directory that is not a post - /audio/, /tags/,\n"
        "    # /feeds/ - falls straight through untouched.\n"
        "    RewriteCond %{DOCUMENT_ROOT}/posts/$1.html -f\n"
